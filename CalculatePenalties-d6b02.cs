...
public abstract class Script_Instance_d6b02 : GH_ScriptInstance
{
  ...
  private void RunScript(Surface externalRegion, List<Rectangle3d> roomRectangles, ref object penalty)
  {
    bool externalRegionIntersect =
      roomRectangles
      .Any(rec => IntersectWithExternalRegion(rec, externalRegion));

    if (externalRegionIntersect)
    {
      penalty = true;
      Print("Some rectangle intersects with external region");
      return;
    }
  ...
  #endregion
  #region Additional
  private bool IntersectWithExternalRegion(Rectangle3d rectangle, Surface region)
  {
    return DoesIntersect(GetRectangleSurface(rectangle), region);
  }
  ...
  private bool IntersectWithOthers(Rectangle3d rectangle, IEnumerable<Rectangle3d> others)
  {
    return
      ...
      .Any(other =>
        DoesIntersect(
          GetRectangleSurface(rectangle),
          GetRectangleSurface(other)
        )
      );
  }
